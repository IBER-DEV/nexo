...
/**
 * La landing vive acá y no en un archivo de ruta porque la sirven dos:
 * `/` (lo que ve quien llega al dominio sin sesión) y `/landing` (la URL
 * histórica, que sigue viva por los enlaces ya publicados en el README,
 * la documentación y fuera del repo).
 */
...
export function LandingPage() {
  ...
  useEffect(() => {
    ...
    // Import dinámico: Lenis toca `window` al construirse, así que no puede
    // evaluarse durante el render del servidor — y ahora esta página sí se
    // renderiza en el servidor, porque es la raíz del dominio.
    import("lenis").then(({ default: Lenis }) => {
      if (cancelled) return;
      lenis = new Lenis({ lerp: 0.1, duration: 1.5 });
